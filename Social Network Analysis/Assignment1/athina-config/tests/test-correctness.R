...
#   args[2]  this tests directory (holds assignment1.R, the reference)
#
...
#    makes levenshteinSim return NaN, not a number
safe_similarity <- function(expr_student, expr_instructor) {
  ...
  # NaN arises when both sides are empty (an unimplemented question). Treat it
  # as no similarity. A non-finite value here is what previously aborted the
  # script with "missing value where TRUE/FALSE needed".
...
# Keep only the questions that were actually evaluated.
results = results[!is.na(results$result), , drop = FALSE]

# Print results and final score
cat("This test evaluates your output with the desired output of a function. Since the correct answer cannot be displayed, you are given instead Levenshtein distance between the string version of the output as an indicator to how close or far your answer is from the desired output. For numeric outcomes, this is not accurate and should still be used as an indicator only.\n")
...
if (nrow(results) == 0) {
  ...
  passed <- nrow(results[results$result == "PASS", , drop = FALSE])
  cat((passed / nrow(results)) * 100)
  cat("\n")
}
